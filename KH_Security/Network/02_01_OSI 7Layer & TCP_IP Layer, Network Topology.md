# OSI 7Layer & TCP/IP Layer & Network Topology
...
|---|---|---|---|---|
| **7계층** | **Application** | 사용자 인터페이스, UI, 응용 프로그램 서비스 제공 | Data | HTTP, FTP, SMTP, 소켓 프로그래밍 |
| **6계층** | **Presentation** | 데이터 형식 설정, 암호화 및 압축 담당 | Data | JPEG, MPEG, SSL/TLS |
| **5계층** | **Session** | 응용 프로세스 간의 통신 세션 유지 및 관리 | Data | RPC, NetBIOS |
| **4계층** | **Transport** | 패킷 순서 및 손상 복구 보장, 수신자 응답 담당 | Segment | **TCP**, UDP |
| **3계층** | **Network** | 논리적 주소(IP) 식별 및 최적의 경로 설정(Routing) | Packet | **IP**, **Router**, ICMP |
| **2계층** | **Data Link** | 하드웨어적 식별(MAC), 프레임 전송, CRC 에러 점검 | Frame | **Switch**, **Bridge**, 랜카드(NIC) |
| **1계층** | **Physical** | 시스템 간 비트 단위의 신호 전송, 케이블 및 물리적인 형태 | Bit | **Repeater**, **Hub**, Cable |
...
## Network Topology와 통신 방식

- 토폴로지는 네트워크 구성 요소들의 물리적 연결 방식을 의미합니다.

### 토폴로지의 종류

![network topology](/KH_Security/Network/img/Topology.png)
...
## Ethernet과 CSMA/CD 메커니즘

- **Ethernet**은 LAN 환경에서 장치들이 MAC 주소를 기반으로 프레임을 주고받기 위해 표준화된 유선 네트워크 기술입니다.

![CSMA/CD](/KH_Security/Network/img/CSMA.CD.png)

### CSMA/CD (Carrier Sense Multiple Access with Collision Detect)
- IEEE 802.3 네트워크에서 매체 사용을 위한 제어 구조 방식입니다.
- **Carrier Sense:** 네트워크 상에 패킷이 있는 지 없는 지 확인 하는 것이다. 
- **Multiple Access:** 보내는 관점으로 보면은 네트워크 상에서 데이터가 없어서 보내다 보니,  
Multiple Access방식으로 보내지고,계속해서 데이터를 보내다보니 모든 스테이션이 데이터를 받은 것이다.  
...
![cd](/KH_Security/Network/img/Collision.png)

### 구간별 상세 설명
1.  **통신의 시작 (Node = 2):**
    * 네트워크 통신은 최소 2개 이상의 노드가 존재할 때 비로소 시작됩니다.
    * 초기에는 노드가 추가될수록 전송되는 데이터 양이 완만하게 증가합니다.

2.  **네트워크 최대치:**
    * 네트워크가 수용할 수 있는 최대 효율 지점입니다. 
    * 이때까지는 노드가 늘어나는 만큼 전체 패킷량도 함께 늘어나지만, Bus형 이더넷을 사용하기 때문에 각 노드가 개별적으로 점유할 수 있는 대역폭은 점차 줄어듭니다.
...
- **Bridge / Switch (L2):** MAC 주소 기반으로 패킷을 필터링합니다. 멀티 포트 브리지가 곧 **스위치 허브**입니다.
    ...
    - Learning: Bridge Table에 없는 항목을 작성해가는 행위
    - Filtering: 특정 포트로 보낼 필요가 없어 필터링 하는 행위
    - Forwarding: Bridge Table을 참고해 해당하는 포트로 보내는 행위
    - Flooding: 패킷을 어디로 보낼지 몰라 모든 포트로 보내는 행위
- **Router (L3):** IP를 이용해 네트워크를 분할하고 연결합니다. RIP, OSPF와 같은 경로 배정 알고리즘을 사용합니다.

![Bridge Table](/KH_Security/Network/img/BridgeTable.png)

### 브릿지의 기능  - 아래에 브릿지의 기능에 대해 정리

- Learnig : 러닝은 이제 패킷의 주소를 보고 학습을 하는 것인데, 예를 들어 A에서 D로 패킷을 보내는데 브릿지에서는 이제 src address를 보고 학습을 한다. 그러면 이제 1번포트에서 온 것을 알기 때문에 브릿지 테이블에 1번 포트에 들어간다.

- Flooding : 러닝하는 과정에서 Destination address를 모르기 때문에 2번포트와 3번 포트에 전부 보내게 되는 과정을 말하고,  시간이 지나다 보면 데이터를 한번 씩 주고 받기때문에 브릿지 테이블이 다 채워질 것입니다.

- Fowarding : 이제 A에서 I로 보낸다고 예를 들면, 이제 브릿지 테이블에 다 들어가 있기 때문에 I가 3번포트에 있는 것을 알게 된다.  
특정 포트로 보내는 것입니다.

- Filtering : 이제 A에서 I로 보내는 과정에서 3번 포트인 것을 알기 때문에 3번포트로는 보내지만 2번 포트로 보내지 않는 것입니다.

---

## 패킷의 지연과 손실

- 브리지, 스위치, 라우터 등의 장치는 패킷이 임시로 대기할 수 있는 버퍼를 가지고 있으며 시간 순으로 저장된 이후 전송되기 때문에 이 과정에서 지연과 손실이 발생할 수 있습니다.

- 이더넷에서는 패킷 자체는 무확인 방식으로 전송되므로 손실이 발생해도 전송 실패를 알 수 없습니다.

- 따라서 이후에 설명할 TCP계층이 관여해 이를 보완합니다
