...
- IP 주소는 단독으로 존재하지 않으며 다음 3가지가 함께 설정되어야 합니다.
  ...
  - Netmask (Subnet Mask):  IP 주소에서 네트워크 영역과 호스트 영역을 구분하는 기준
    - IP주소 내에서 어디까지가 네트워크를 알려주는 마킹된 정보
  - Gateway: 다른 네트워크로 나갈 때 반드시 거쳐야 하는 출입구 역할의 장치
    - 라우터의 우리쪽 포트라고 할 수 있고, 다른 네트워크와 통신을 하기위해 필요합니다.

---

## Class 기반 IP 주소 체계

- Class는 현재 기술적 스펙이 아닌 과거 관리 체계입니다. 별 다른 의미는 없습니다.
...
- IP 주소는 sequence number가 아니라 address 입니다.
- Netmask는 IP 주소에서 Network 영역과 Host 영역을 구분해 줍니다.
...
![DIV1](/KH_Security/Network/img/Network_div1.png)
![DIV2](/KH_Security/Network/img/Network_div2.png)
